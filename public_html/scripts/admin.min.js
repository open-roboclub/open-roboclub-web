...
function(){updateProfile(c.value,e.options.item(e.options.selectedIndex).value);return!1}}completed=!1;function pushNewsToDatabase(a){progress(!0);firebase.database().ref("news/").push(a).then(function(){completed&&progress(!1);alert("News posted successfully")})["catch"](function(a){completed&&progress(!1);alert("An error occurred! \n"+a+"\n You do not have permission to send notiifcatons")})}
function requestFCM(a,b){var d={title:a,message:b};progress(!0);firebase.auth().currentUser.getToken().then(function(a){$.ajax({type:"POST",url:"./send_notification",headers:{Authorization:"Bearer "+a},crossDomain:!1,data:d,dataType:"json",success:function(a,c,b){completed=!0;progress(!1);a.error?alert(a.message):alert(a.message+"\nMessage ID : "+a.message_id)},error:function(a,c,b){completed=!0;progress(!1);alert("Error "+b)}})})["catch"](function(a){completed=!0;progress(!1);alert(a)})}
function sendNotification(a,b,d){if(null==a||null==b)alert("Can't send empty message");else if(4>a.length||5>b.length)alert("Title or Message is too small");else{var c={notice:b,date:(new Date).toDateString()};null!=d&&5<d.length&&(c.link=d);completed=!1;pushNewsToDatabase(c);requestFCM(a,b)}}
...
toggleVisibility(!0,d);populateOptions(c)}else a.textContent="Sign in",b.textContent="You're signed out!",a.onclick=function(){location.href="./signin"},toggleVisibility(!1,d)},function(a){console.log(a)})}window.addEventListener("load",function(){initApp()});